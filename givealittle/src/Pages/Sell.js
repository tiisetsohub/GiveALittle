...
export default function Sell() {
    ...
    const [newImg1, setnewImg1] = useState("");                   //state for image
    ...
    const itemRef = collection(db, "SellItems");            //refernce for item


    const addItem = async () => {           //handles adding an item to database
        await addDoc(itemRef, {Name: newName, Description: newDescription, Price: newPrice, Quantity: newQuantity, Image1: newImg1, Image2: newImg2, Image3: newImg3, Seller: name });
        alert("Added") 
    ...
    return (
        <div className="bigdiv">
            ...
            <div className='logindiv'>          {/*form for item information*/}
                ...
                <br />
                <input className="edtimg1" id="input" placeholder="Image Link" onChange={(event) => {
                    setnewImg1(event.target.value)
                }} />
                <br />
                <br />
                <input className="edtimg2" id="input" placeholder="Image Link" onChange={(event) => {
                    setnewImg2(event.target.value)
                }} />
                <br />
                <br />
                <input className="edtimg3" id="input" placeholder="Image Link" onChange={(event) => {
                    setnewImg3(event.target.value)
                }} />
                <br />
